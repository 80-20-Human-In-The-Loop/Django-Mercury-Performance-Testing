...
def parse_coverage_summary(coverage_file):
    ...
    try:
        ...
        source_files = ['common.c', 'query_analyzer.c', 'metrics_engine.c', 
                       'test_orchestrator.c', 'performance_monitor.c']
...
def print_colored_summary(coverage_data):
    ...
    for item in coverage_data:
        ...
        print(f"File '{file_name}' {color}{percentage:6.2f}%{RESET} ({item['covered']}/{item['total']} lines)")
...
def main():
    ...
    if coverage_data:
        ...
        print(f"{BOLD}Overall Coverage: {color}{overall_percentage:.1f}%{RESET} ({covered_lines}/{total_lines} lines)")
