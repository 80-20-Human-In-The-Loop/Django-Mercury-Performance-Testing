...
def show_final_summary(display: "MercuryVisualDisplay", failures: int) -> None:
    ...
    if failures == 0:
        summary.append(
            f"{SYMBOLS['check']} ALL TESTS PASSED!\n\n", style="bold green"
        )
    else:
        summary.append(
            f"{SYMBOLS['cross']} {failures} TESTS FAILED\n\n", style="bold red"
        )
...
def _add_mercury_info(display: "MercuryVisualDisplay", summary: Text) -> None:
    ...
    if display.stats["mercury_tests"] > 0:
        ...
        has_meaningful_metrics = (
            display.metrics_tracker.mercury_test_count > 0 and
            (display.metrics_tracker.grade_distribution or 
             display.metrics_tracker.test_metrics)
        )
        ...
        worst_performers = display.metrics_tracker.get_worst_performers(
            limit=10
        )
        if worst_performers:
            summary.append(
            ...
            for i, worst in enumerate(worst_performers):
                ...
                if ":" in worst:
                    parts = worst.split(":")
                    test_part = (
                        parts[0].strip().lstrip("❌").lstrip("⚠️").strip()
                    )
            ...
            if display.hints_enabled and display.profile == 'student':
...
def _add_isolation_issues(
    display: "MercuryVisualDisplay", summary: Text
) -> None:
    ...
    if display.failed_tests:
        ...
        for test in display.failed_tests[:10]:  # Show first 10
            if "[ISOLATION]" in test:
                ...
                summary.append(
                    f"  • {clean_test} {SYMBOLS['warning']}\n", style="yellow"
                )
            else:
                summary.append(f"  • {test}\n")
        if len(display.failed_tests) > 10:
            summary.append(
                f"  ... and {len(display.failed_tests) - 10} more\n"
            )
...
def _add_contextual_learn_more(display, summary: Text) -> None:
    ...
    if not hasattr(display, 'shown_tip_categories') or not display.shown_tip_categories:
        return
    
    summary.append(f"\n{SYMBOLS['bulb']} Continue Learning:\n", style="bold yellow")
    
    # Generate learn more commands based on shown categories
    ...
    learn_more_commands.update([
        ...
        'mercury-test --learn performance-testing'
    ])
