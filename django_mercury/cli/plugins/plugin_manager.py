...
class PluginManager:
    ...
    def _load_from_config(self, config: Dict[str, Any]) -> None:
        ...
        plugin_file_map = {
            'learn': 'plugin_explain.py',  # The learn plugin is in plugin_explain.py
        }
    ...
    def _load_plugin_file(self, plugin_file: Path) -> None:
        ...
        for name, obj in inspect.getmembers(module, inspect.isclass):
            if (issubclass(obj, MercuryPlugin) and 
                obj is not MercuryPlugin and
    ...
    def list_plugins(self) -> str:
        """Get a formatted list of all available plugins.
        ...
        lines = ["Available Mercury-Test Plugins:", ""]
        
        for plugin in self.plugins:
            status = "✅" if plugin else "❌"
            lines.append(f"{status} {plugin.name} (priority: {plugin.priority})")
