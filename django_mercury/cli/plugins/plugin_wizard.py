...
class CommandStorage:
    ...
    def save_command(self, name: str, command: str, description: str = "", tags: List[str] = None) -> bool:
    ...
    def get_recent_command(self) -> Optional[Dict]:
        ...
        sorted_cmds = sorted(
            commands,
            key=lambda x: x.get('stats', {}).get('last_used', ''),
            reverse=True
        )
...
class MenuSystem:
    ...
    def select_option(self, title: str, options: List[Tuple[str, str, str]], 
    ...
    def _rich_menu(self, title: str, options: List[Tuple[str, str, str]], 
                   allow_back: bool) -> Optional[str]:
    ...
    def _simple_menu(self, title: str, options: List[Tuple[str, str, str]], 
                     allow_back: bool) -> Optional[str]:
...
class TestDiscovery:
    ...
    def _is_local_app(self, app_name: str) -> bool:
        ...
        third_party = ['django', 'rest_framework', 'corsheaders', 'debug_toolbar', 
                      'crispy_forms', 'allauth', 'channels', 'celery']
                ...
                if py_file.name != '__init__.py':
                    ...
                    try:
                        ...
                        if any(indicator in content for indicator in 
                               ['TestCase', 'def test_', 'class Test', '@pytest']):
            ...
            for test_file in test_files:
                try:
                    ...
                    for line_num, line in enumerate(lines, 1):
                        ...
                        if stripped.startswith('class ') and ('Test' in stripped or 'test' in stripped.lower()):
                        ...
                        # Look for test methods
                            ...
                            if query_lower in method_match.lower() and current_class:
                                ...
                                rel_path = test_file.relative_to(app_path)
                                module_path = str(rel_path.with_suffix('')).replace('/', '.')
                                full_path = f"{app_name}.{module_path}.{current_class}.{method_match}"
...
class TestWizard:
    ...
    def find_django_project(self, args: Namespace) -> bool:
        ...
        if not manage_py:
            ...
            if self.menu.use_rich:
                ...
                self.menu.console.print("[dim]Try running from your project directory or use --search-deep[/dim]")
            else:
    ...
    def select_action(self) -> Optional[str]:
        ...
        options = [
            ('all_tests', '🌍 Run all tests', 
             'Test everything (might take a while)'),
            ('app_tests', '📦 Test specific app',
             'Choose which app to test'),
            ('file_tests', '📄 Test single file',
             'Test one specific test file'),
            ('function_tests', '🎯 Test specific function',
        ...
        if saved_count > 0:
            options.append(
                ('saved_commands', f'📚 Saved commands ({saved_count} in project)',
                 'Run or manage saved test configs')
            )
        else:
            options.append(
                ('saved_commands', '📚 No saved commands yet',
    ...
    def handle_saved_commands(self) -> Optional[str]:
        ...
        if not commands:
            if self.menu.use_rich:
                ...
                self.menu.console.print("[dim]Build and save your first command to get started.[/dim]")
            else:
    ...
    def select_test_modules(self, app: str) -> Optional[List[str]]:
        ...
        if self.menu.use_rich:
            ...
            for i, (subdir, count) in enumerate(module_info, 1):
                self.menu.console.print(f"  {i}. {subdir} ({count} test{'s' if count != 1 else ''})")
            self.menu.console.print()
            
            # Get selection
            from rich.prompt import Prompt
            selection = Prompt.ask(
                "Enter module numbers to test (e.g., 1,3,5 or * for all)",
                default="*"
            )
        else:
            print(f"\n📦 Test modules in {app}:")
        ...
        if self.menu.use_rich:
            ...
            for idx in selected_indices:
                ...
                self.menu.console.print(f"  • {app}.tests.{subdir} ({count} test{'s' if count != 1 else ''})")
            self.menu.console.print(f"[dim]Total: {total_tests} test{'s' if total_tests != 1 else ''}[/dim]\n")
        else:
            print("\nThis will test:")
            for idx in selected_indices:
                subdir, count = module_info[idx]
    ...
    def select_test_subdirectory(self, app: str, structure: Dict[str, Any]) -> Optional[str]:
        ...
        if structure['root_files']:
            ...
            options.append(('root', f"📄 Root level tests", f"{count} file{'s' if count != 1 else ''}"))
        
        # Add option to show all files
        total_files = sum(len(files) for files in structure['subdirs'].values()) + len(structure['root_files'])
        options.append(('all', f"📋 Show all files", f"{total_files} files total"))
        
        return self.menu.select_option(
            f"📂 Select test category in {app}:",
            options,
            allow_back=True
        )
    ...
    def select_test_function(self) -> Optional[str]:
        ...
        try:
            ...
            if self.menu.use_rich:
                ...
                self.menu.console.print("[dim]Example: users.tests.test_models.TestUser.test_creation[/dim]")
    ...
    def _handle_search_results(self, found_tests: List[Dict[str, Any]], query: str) -> Optional[str]:
    ...
    def _select_specific_search_results(self, found_tests: List[Dict[str, Any]]) -> Optional[str]:
        ...
        if self.menu.use_rich:
            ...
            for i, test in enumerate(found_tests, 1):
                icon = "🏷️" if test['type'] == 'class' else "🔹"
                self.menu.console.print(f"  {i}. {icon} [bold]{test['name']}[/bold] [dim]({test['app']}/{test['file']})[/dim]")
    ...
    def build_command(self, action: str, target: Optional[str], 
            ...
            if not command_args:
                ...
                if len(test_targets) == 1:
                    ...
                    if len(test_targets) > 5:
                        self.menu.console.print(f"[dim]  ... and {len(test_targets) - 5} more[/dim]")
