...
class SmartDiscoveryPlugin(MercuryPlugin):
    ...
    def register_arguments(self, parser: ArgumentParser) -> None:
        ...
        parser.add_argument(
            '--no-cache',
            action='store_true',
            help='Skip location caching for this run'
        )
        
        parser.add_argument(
            '--clear-cache',
            action='store_true',
            help='Clear cached manage.py locations and exit'
        )
    ...
    def _search_current_and_parents(self, start_dir: Path, show_search: bool = False) -> Optional[Path]:
        ...
        for level in range(5):  # Check up to 5 levels up
            ...
            if manage_path.exists():
                if self._verify_manage_py(manage_path):
                    if show_search:
                        location = "Current directory" if level == 0 else f"Parent directory (level {level})"
                    ...
                    if show_search:
                        location = "Current directory" if level == 0 else f"Parent directory (level {level})"
                        self._show_search_result(location, "⚠️", f"Invalid manage.py at {manage_path}")
    ...
    def _search_current_directory_patterns(self, current_dir: Path, show_search: bool = False) -> Optional[Path]:
            ...
            for item in current_dir.iterdir():
                if item.is_dir():
                    ...
                    if manage_path.exists():
                        ...
                        if self._verify_manage_py(manage_path):
                            if show_search:
                                self._show_search_result("Current directory subdirectories", "✅", manage_path)
                            return manage_path
                        else:
                            found_any = True
                            self.invalid_files_found.append(manage_path)
                            if show_search:
                                self._show_search_result("Current directory subdirectories", "⚠️", f"Invalid manage.py at {manage_path}")
        ...
        for pattern in common_patterns:
            ...
            if pattern_path.exists():
                ...
                if self._verify_manage_py(pattern_path):
                    if show_search:
                        self._show_search_result(f"Current directory pattern ({pattern})", "✅", pattern_path)
                    return pattern_path
                else:
                    found_any = True
                    self.invalid_files_found.append(pattern_path)
                    if show_search:
                        self._show_search_result(f"Current directory pattern ({pattern})", "⚠️", f"Invalid manage.py at {pattern_path}")
                ...
                for match in matches:
                    if match.exists():
                        ...
                        if self._verify_manage_py(match):
                            if show_search:
                                self._show_search_result(f"Current directory pattern ({pattern})", "✅", match)
                            return match
                        else:
                            found_any = True
                            self.invalid_files_found.append(match)
                            if show_search:
                                self._show_search_result(f"Current directory pattern ({pattern})", "⚠️", f"Invalid manage.py at {match}")
            except (PermissionError, OSError):
                continue
    ...
    def _search_sibling_patterns(self, start_dir: Path, show_search: bool = False) -> Optional[Path]:
            ...
            if pattern_dir.is_dir():
                ...
                if manage_path.exists():
                    if self._verify_manage_py(manage_path):
                        ...
                        self.invalid_files_found.append(manage_path)
                        if show_search:
                            self._show_search_result("Sibling app directories", "⚠️", f"Invalid manage.py at {manage_path}")
    ...
    def _search_children_with_progress(self, start_dir: Path, show_search: bool = False) -> Optional[Path]:
    ...
    def _search_children_rich_progress(self, start_dir: Path) -> Optional[Path]:
        ...
        self._show_search_result("Subdirectories", "❌", f"Searched {len(dirs_to_search)} directories")
        return None
    ...
    def _get_searchable_dirs(self, start_dir: Path):
        ...
        exclude_dirs = {
            ...
            'node_modules', '.tox', '.pytest_cache', 'htmlcov', '.coverage',
            '.mypy_cache', '.idea', '.vscode', 'build', 'dist', '*.egg-info'
        }
