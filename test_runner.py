...
class TimedTextTestResult(unittest.TextTestResult):
    ...
    def __init__(self, stream, descriptions, verbosity, ci_mode=False) -> None:
        ...
        self.module_color_stats: Dict[str, Dict[str, int]] = defaultdict(
            lambda: {"GREEN": 0, "YELLOW": 0, "RED": 0, "CRITICAL": 0}
        )
...
class TimedTextTestRunner(unittest.TextTestRunner):
    ...
    def _analyze_color_distribution(self, result, total_time):
        ...
        for color, count in [
            ("CRITICAL", result.color_counts["CRITICAL"]),
    ...
    def _get_performance_insights(self, result):
        ...
        if result.color_counts["CRITICAL"] > 0:
            ...
            critical_tests = sorted(
                result.color_tests["CRITICAL"], key=lambda x: x[1], reverse=True
            )
...
def run_tests_with_coverage(
    test_modules: List[str], verbose: bool = False, no_timing: bool = False, ci_mode: bool = False
) -> bool:
...
def run_tests_without_coverage(
    ...
    for module_name in test_modules:
        try:
            ...
            if "No module named" in str(e):
                print(
                    f"   [HINT] Make sure the module path is correct and all required packages are installed"
                )
        except Exception as e:
            ...
            if "false" in str(e).lower() and "not defined" in str(e).lower():
                print(
                    f"   [SYNTAX HINT] Check for lowercase 'false/true' instead of 'False/True' in Python code"
                )
    ...
    if no_timing:
        ...
        runner = TimedTextTestRunner(
            verbosity=verbosity, buffer=False, timeout_seconds=5.0, ci_mode=ci_mode
        )
...
def run_c_integration_tests(verbose: bool = False) -> bool:
    ...
    try:
        test_script = (
            PERFORMANCE_TESTING_ROOT / "tests" / "bindings" / "test_c_integration_simple.py"
        )
...
def check_and_build_c_libraries(skip_build: bool = False, ci_mode: bool = False) -> bool:
    ...
    # Required libraries
    required_libs = ["libquery_analyzer.so", "libmetrics_engine.so", "libtest_orchestrator.so"]
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if not still_missing:
                ...
                if not ci_mode:
                    print(
                        f"⚠️  Build completed but some libraries still missing: {', '.join(still_missing)}"
                    )
...
def setup_test_environment():
    ...
    # c_bindings logger removed - pure Python mode
    ...
    # logging.getLogger("performance_testing.thread_safety").setLevel(logging.ERROR)  # Removed - thread_safety module deprecated
    ...
    # Pure Python mode - no initialization needed
    if os.environ.get("MERCURY_DEFER_INIT", "0") == "1":
        print("✅ Django Mercury running in pure Python mode")
...
def main():
    ...
    try:
        ...
        parser = argparse.ArgumentParser(description="Performance Testing Framework Test Runner")
        parser.add_argument(
        ...
        # Apply quiet mode - reduce verbosity unless explicitly verbose
        if args.quiet and not args.verbose:
            # In quiet mode, use minimal verbosity
            verbose_for_tests = False
        else:
            verbose_for_tests = args.verbose

        # Setup environment
        setup_test_environment()

        # Show startup messages unless in quiet mode
        ...
        if args.module:
            ...
            module_success = run_specific_module_tests(
            ...
            if args.coverage:
                python_success = run_tests_with_coverage(
                    test_modules, verbose_for_tests, args.no_timing, ci_mode
                )
            else:
                python_success = run_tests_without_coverage(
                    test_modules, verbose_for_tests, args.no_timing, ci_mode
                )
